...
function Update() {
  ...
  const [form, setForm] = useState({
    ...
    //Params : 주소에 g_code를 추가하기 위함.
  const navigate = useNavigate();

  //2. 서버측에 넘길 데이터(g_code)를 통신해서 성공, 실패여부 출력
  useEffect(()=>{
    axios.get(`https://port-0-backend2025-mbeeobco2e6ef2af.sel4.cloudtype.app/goods/${g_code}`)
    //성공이면 출력
    .then(res=>{
  ...
  const handleSubmit=(e)=>{  
    ...
    axios.put(`https://port-0-backend2025-mbeeobco2e6ef2af.sel4.cloudtype.app/goods/update/${g_code}`,{
